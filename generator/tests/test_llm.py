...
from ctx_generator.llm import LLMClient, LLMError
...
def test_generate_returns_message_content():
    ...
    with patch("ctx_generator.llm.requests.post", return_value=make_response(payload=payload)) as post:
...
def test_generate_raises_on_http_error():
    with patch("ctx_generator.llm.requests.post", return_value=make_response(status=500, text="boom")):
        with pytest.raises(LLMError, match="500"):
            client().generate("sys", "user")


def test_generate_raises_on_unexpected_shape():
    with patch("ctx_generator.llm.requests.post", return_value=make_response(payload={"oops": True})):
...
def test_generate_raises_on_empty_content():
    ...
    with patch("ctx_generator.llm.requests.post", return_value=make_response(payload=payload)):
